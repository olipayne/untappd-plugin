...
if ( ! class_exists( 'Untappd_Cron' ) ) {
	 ...
	 */
	class Untappd_Cron extends Untappd_Module {
		...
		public static function add_custom_cron_intervals( $schedules ) {
			$schedules[ 'untappd_debug' ] = array(
				'interval' => 5,
				'display'  => 'Every 5 seconds'
			);

			$schedules[ 'untappd_ten_minutes' ] = array(
				'interval' => 60 * 10,
				'display'  => 'Every 10 minutes'
			);

			$schedules[ 'untappd_example_interval' ] = array(
				'interval' => 60 * 60 * 5,
		...
		public static function fire_job_at_time() {
			...
			if ( (int) date( 'G', $now ) >= 1 && (int) date( 'G', $now ) <= 3 ) {
				if ( ! get_transient( 'untappd_cron_example_timed_job' ) ) {
					//Untappd_CPT_Example::exampleTimedJob();
					set_transient( 'untappd_cron_example_timed_job', true, 60 * 60 * 6 );
				}
			}
		}
		...
		public function register_hook_callbacks() {
			add_action( 'untappd_cron_timed_jobs',  __CLASS__ . '::fire_job_at_time' );
			add_action( 'untappd_cron_example_job', __CLASS__ . '::example_job' );
		...
		public function activate( $network_wide ) {
			if ( wp_next_scheduled( 'untappd_cron_timed_jobs' ) === false ) {
				wp_schedule_event(
					current_time( 'timestamp' ),
					'untappd_ten_minutes',
					'untappd_cron_timed_jobs'
				);
			}

			if ( wp_next_scheduled( 'untappd_cron_example_job' ) === false ) {
				wp_schedule_event(
					current_time( 'timestamp' ),
					'untappd_example_interval',
					'untappd_cron_example_job'
				);
			}
		}
		...
		public function deactivate() {
			wp_clear_scheduled_hook( 'untappd_cron_timed_jobs' );
			wp_clear_scheduled_hook( 'untappd_cron_example_job' );
		}
