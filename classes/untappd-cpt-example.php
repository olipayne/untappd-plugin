...
if ( ! class_exists( 'Untappd_CPT_Example' ) ) {

	/**
	 * Creates a custom post type and associated taxonomies
	 */
	class Untappd_CPT_Example extends Untappd_Module implements Untappd_Custom_Post_Type {
		protected static $readable_properties  = array();
		protected static $writeable_properties = array();

		const POST_TYPE_NAME = 'Untappd Custom Post Type';
		const POST_TYPE_SLUG = 'untappd-cpt';
		const TAG_NAME       = 'Untappd Custom Taxonomy';
		const TAG_SLUG       = 'untappd-custom-tax';
		...
		protected static function get_post_type_params() {
			...
			return apply_filters( 'untappd_post-type-params', $post_type_params );
		}
		...
		protected static function get_tag_taxonomy_params() {
			...
			return apply_filters( 'untappd_tag-taxonomy-params', $tag_taxonomy_params );
		}
		...
		public static function markup_meta_boxes( $post, $box ) {
			...
			switch ( $box['id'] ) {
				case 'untappd_example-box':
					$variables['exampleBoxField'] = get_post_meta( $post->ID, 'untappd_example-box-field', true );
					$view                         = 'untappd-cpt-example/metabox-example-box.php';
					break;

				/*
				case 'untappd_some-other-box':
					$variables['someOtherField'] = get_post_meta( $post->ID, 'untappd_some-other-field', true );
				 	$view                        = 'untappd-cpt-example/metabox-another-box.php';
					break;
				*/
		...
		public static function is_protected_meta( $protected, $meta_key, $meta_type ) {
			switch( $meta_key ) {
				case 'untappd_example-box':
				case 'untappd_example-box2':
					$protected = true;
					break;

				case 'untappd_some-other-box':
				case 'untappd_some-other-box2':
					$protected = false;
					break;
			}
		...
		protected static function save_custom_fields( $post_id, $new_values ) {
			if ( isset( $new_values[ 'untappd_example-box-field' ] ) ) {
				if ( false ) { // some business logic check
					update_post_meta( $post_id, 'untappd_example-box-field', $new_values[ 'untappd_example-box-field' ] );
				} else {
		...
		/**
		 * Defines the [untappd-cpt-shortcode] shortcode
		 *
		 * @mvc Controller
		 *
		...
		public static function cpt_shortcode_example( $attributes ) {
			$attributes = apply_filters( 'untappd_cpt-shortcode-example-attributes', $attributes );
			$attributes = self::validate_cpt_shortcode_example_attributes( $attributes );

			return self::render_template( 'untappd-cpt-example/shortcode-cpt-shortcode-example.php', array( 'attributes' => $attributes ) );
		}
		...
		protected static function validate_cpt_shortcode_example_attributes( $attributes ) {
			...
			return apply_filters( 'untappd_validate-cpt-shortcode-example-attributes', $attributes );
		}
		...
		protected static function get_default_cpt_shortcode_example_attributes() {
			...
			return apply_filters( 'untappd_default-cpt-shortcode-example-attributes', $attributes );
		}
